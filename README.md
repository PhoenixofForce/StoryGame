...
Set the dev-profile as an environment variable 
- in your run configuration `spring.profiles.active=dev` or
- as an JVM parameter `-Dspring.profiles.active=dev`
This generates the necessary message types automatically in the frontend.
...
When changing a file, the page gets rebuilt automatically and shows you the new version.
...
```bash
docker build -f service.Dockerfile -t Your_Image_Name_here .
```
...
This generates the `src_ui\dist` folder, which contains the static html file.
Build the docker image with the following command

```bash
docker build -f ui.Dockerfile -t Your_Image_Name_here .
```

### Deploying Frontend

The easiest way to deploy this is to run in the `src_ui\dist` Folder to instantly provide the generated output.
```bash
python3 -m http.server
```

Otherwise, pull and run the docker file with the following commands
```bash
...
- If you want to **contribute code** open a Pull Request (to the develop branch) with a meaningful description. Please refrain from opening a pull request that only contains cosmetic changes.
